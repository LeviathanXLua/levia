local linetoken = "9bdGv5eDYDRqlKGBFlhwEGgB70jTCFWLacIP88fxzBv"
...
function FNcheck()
    kaitunkrob = false
    kaitunkardkeaw = false
    
    if BDFN and DFFN and MNFN and YMFN and HSFN then
        kaitunkrob = true
        TextFN.Position = UDim2.new(0, 105, 0, 235)
        TextFN.TextColor3 = Color3.fromHex("#64ff64")
        TextFN.Text = "ไก่โคตรครบ ปิดเลย"
    elseif BDFN and DFFN and MNFN and YMFN then
        kaitunkardkeaw = true
        TextFN.Position = UDim2.new(0, 220, 0, 235)
        TextFN.TextColor3 = Color3.fromHex("#ff3232")
        TextFN.Text = "ไก่ครบขาดแค่เคียว"
    end
    
    if kaitunkrob then
        kaitunformat = "ไก่โคตรครบ ฟูลมาก"
    end
    
    if kaitunkrob then
        notiline(game.Players.LocalPlayer.Name .. "\n" .. kaitunformat .. "\nไก่ของครบหมดแล้ว ปิดให้แล้วด้วย")
        game:Shutdown()
    end
end
